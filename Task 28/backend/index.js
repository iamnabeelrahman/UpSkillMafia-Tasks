...
app.get("/", function (req, res) {
  ...
  res.send(
    `<h1> Welcome to the Todo API!</h1> </br> ${todo} </br>${todos} </br>${completed} </br>${delete1}`
  );
});
...
// Endpoint for marking todo as completed.
...
app.get("/search", async (req, res) => {
  ...
  try {
    const results = await todo.find({
      title: new RegExp(parsedTitle.data.title, "i"),
